...
class TradingExchange(Exchange):
    ...
    async def buy(self, exchange_instance: CcxtProExchange, coin_name: str, usdt_quantity: float | None = None, usdt_name: str = 'USDT'):
        ...
        if not exchange_instance.has['createMarketOrder']:
            self.logger.warning(f"Market buy is not supported on exchange {exchange_instance.id}")
            return None

        symbol = f"{coin_name}/{usdt_name}"
        try:
            order = await exchange_instance.create_order(symbol, 'market', 'buy', usdt_quantity)
            filled_amount = order.get('filled')
            cost = order.get('cost')
            self.logger.info(f"Buy order: {symbol}, cost: {cost}, filled: {filled_amount}")
            return order
        except Exception as e:
