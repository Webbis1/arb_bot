...
class Exchange(ABC, ExchangeModel):
    ...
    #Trader
    @abstractmethod
    async def sell(self, coin: COIN_ID, amount: float | None = None) -> None: ...
    @abstractmethod
    async def buy(self, coin: COIN_ID, amount: float | None = None) -> None: ...
    
    @abstractmethod
    async def get_deposit_address(self, coin_name: str, chain: str) -> str: ...
